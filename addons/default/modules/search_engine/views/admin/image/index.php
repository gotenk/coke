<?php if (isset($current_tree) && $current_tree ): ?>
		 <script type="text/javascript">
		 	window.parent.idImagePicture ='<?php echo $current_tree; ?>';
		 </script>
<?php		endif;
	?>
<div id="upload-box">
	<h2><?php echo lang('files:upload'); ?><span class="close ui-icon ui-icon-closethick"><?php echo lang('buttons.close'); ?></span></h2>
	<?php echo form_open_multipart('admin/instructor/upload'); ?>
		<?php if (isset($current_tree) && $current_tree) :
					echo form_hidden('current_tree',$this->uri->segment(5)); 
			   endif;
			   echo form_hidden('redirect_to', 'image'); 
								?>
		<?php echo form_hidden('is_banner_image', $is_banner_image); ?>
		<ul>
			
				<?php echo form_input('name', set_value('name'), 'id="name" style="display:none;" '); ?>
			<li>
			...
			<li>
				...
				<?php echo form_dropdown('folder_id',$folders_tree, 'id="folder"'); ?>
			</li>
			<li>
			...
			<li>
				<?php echo form_submit('button_action', lang('buttons:upload'), 'class="button"'); ?>
...
<div id="files_browser">
	<div id="files_left_pane">
		...
		<?php if ( ! $folder->parent_id): ?>
			<?php if($current_folder->id == $folder->id ) : ?>
			<li id="folder-id-<?php echo $folder->id; ?>" class="<?php echo $current_folder && $current_folder->id == $folder->id ? 'current' : ''; ?>">
				<?php echo anchor("admin/instructor/image/index/{$folder->id}", $folder->name, 'title="'.$folder->slug.'"'); ?>
			</li>
			<?php endif;?>
		<?php endif; ?>
		<?php endforeach; ?>
		<?php if ($folders): ?>
			<li class="upload">
				<?php echo anchor("admin/instructor/image/#upload", lang('files:upload'), 'title="upload"'); ?>
			</li>
		<?php endif; ?>
		</ul>
	</div>
	<div id="files_right_pane">
		...
		<?php if ($current_folder): ?>
			...
			<div id="slider"></div>-->
			<!-- folder contents -->
			<input id="insert_width" data-name="<?php echo lang('wysiwyg.label.no_limit'); ?>" type="hidden" name="insert_width" value="200" />
			...
			<table class="table-list" border="0">
				...
				<tbody>
					...
					<tr class="<?php echo alternator('', 'alt'); ?>">
						<td class="image"><img <?php 	if ($is_thumb) :  ?>class="pyro-image" <?php endif; ?> src="<?php echo site_url($image->location === 'local' ? 'files/thumb/'.$image->id.'/100/100' : 'files/cloud_thumb/'.$image->id); ?>" alt="<?php echo $image->name; ?>" width="100" onclick="javascript:<?php echo ($is_banner_image)?  insertImageBanner :insertImage ; ?>('<?php echo $image->id."', '".htmlentities($image->name)."', '".$image->location."', '".$image->path; ?>');" /></td>
						...
						<td class="meta size"><?php echo anchor(site_url('admin/instructor/image/delete/'.$current_folder->id.'/'.$image->id),'Delete','title="Delete"'); ?></td>
					</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
