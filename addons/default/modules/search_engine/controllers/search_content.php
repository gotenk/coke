...
class Search_content extends Public_Controller {
	...
	private function crawlInstagram(){
	    ...
	    $this->db->limit(1);
	    $info_insta = $this->db->get_where('search_engine_data');
	    $entity_id = 0;
		...
		if(isset($media['data']))
		{
		    foreach($media['data'] as $insta){
				...
				//var_dump($insta);
		        if ($insta['type'] == 'video' && $this->instagram_type_source=='video'){
		            $type= 'video';
		        ...
		        /*} else {
				...
				else if ($insta['type'] == 'image' && $this->instagram_type_source=='image') {
					//data instagram 
					...
					$data_insta = array(
			        	...
			        	'name'			=> ($this_name)? $this_name :  $screen_name,
						'url'			=> $insta['link'],
						'lat'			=> isset($insta['location']['latitude'])? $insta['location']['latitude'] :'',
						'lng'			=> isset($insta['location']['longitude'])? $insta['location']['longitude'] :'',
						'location'		=> isset($insta['location']['name'])? $insta['location']['name'] :'',
						//'video_preview' => $media_url_video_preview,
			        	'via'		=> 'instagram',
						'status'	=>'draft'
			        );
					...
					//update image name
					$id = $this->db->insert_id();
					$new_imagename = $this->change_image($id,$media_url);
					$new_picture= $this->grab_picture_twitter($source_id,$media_url,$new_imagename);
					//user photo
					$new_imagename_photo = $this->change_image($id,$user_photo);
					$new_user_photo = $this->grab_picture_twitter($source_id,$user_photo,$new_imagename_photo);
					$this->search_content_m->update_by(array('id'=>$id),array('picture'=>$new_picture,'photo_profile'=>$new_user_photo));
					//end update image name
				}
			}	
	    }
		...
		if(isset($media['pagination']))
		{
			 ...
			 $result_status = $this->search_engine_status_m->get_by(array('via'=>'instagram'));
			 if($result_status)
			 {
				...
				$media['pagination']['min_tag_id'] = (intval($media['pagination']['min_tag_id'])  < intval($result_status->max_id))? intval($media['pagination']['min_tag_id']) :intval($result_status->max_id); 
				 $this->search_engine_status_m->update_by(array('via'=>'instagram'),array('max_id'=>$media['pagination']['next_max_tag_id'],'since_id'=>$media['pagination']['min_tag_id']));
			 }
			 else {
			 	$this->search_engine_status_m->insert(array('max_id'=>((isset($media['pagination']['next_max_tag_id']))?$media['pagination']['next_max_tag_id'] :NULL),'since_id'=>((isset($media['pagination']['min_tag_id']))?$media['pagination']['min_tag_id'] :NULL),'via'=>'instagram'));
			 }
	...
	private function search_vine()
	{
		...
		//check update
		$data_video_status= $this->search_engine_status_m->get_by(array('via'=>'vine'));
		...
		if($since_id && $max_id)
		{
			...
			if($total_page != $total_page_db)
			{
				...
				$this->search_engine_status_m->update_by(array('via'=>'vine'),array('max_id'=>$max_id,'total_page'=>$total_page,'since_id'=>$since_id ));
			...
			if(isset($data_since->records) && count($data_since->records))
			{
				...
				{
					$since_id = $since_id-1;
					$this->search_engine_status_m->update_by(array('via'=>'vine'),array('since_id'=>$since_id));
				}
			...
			if(isset($data_max->records) && count($data_max->records))
			{
				...
				{
					$max_id =$max_id+1;
					$this->search_engine_status_m->update_by(array('via'=>'vine'),array('max_id'=>$max_id));
				}
			}
			...
			{
				 $result_status = $this->search_engine_status_m->get_by(array('via'=>'vine'));
				 if($result_status)
				 {
					 $this->search_engine_status_m->update_by(array('via'=>'vine'),array('max_id'=>$current_max_id,'total_page'=>$total_page,'since_id'=>$current_since_id));
				 }
				 else {
				 	$this->search_engine_status_m->insert(array('max_id'=>$current_max_id,'since_id'=>$current_since_id,'total_page'=>$total_page,'via'=>'vine'));
				}
			}
		}
	...
	private function search_hashtag()
	{
	   ...
	   // if( $this->input->is_cli_request() )
		//{
			...
			$data_video_status= $this->search_engine_status_m->get_by(array('via'=>'twitter'));
			 //maximum		
			 $since_id = NULL;
			...
			if($since_id != NULL && $max_id != NULL)
			{
				//get max id
				//&mode=videos
					$getfield = '?q='.rawurlencode($this->global_tag).'&since_id='.$since_id.'&include_entities=1&count='.$max_count;
					$requestMethod = 'GET';
					...
					$_since_id = $data->{'search_metadata'}->{'since_id'} ;
				 	 ...
				 	 foreach($data->{'statuses'} as $data_insert)
					 {
					 	...
					 	if (!$this->filter_tag($data_insert->text,$filter_text)) continue;
								...
								if(isset($data_insert->place->full_name))
								{
									$place = $data_insert->place->full_name;
								}
								...
								$this->search_content_m->update_by(array('id'=>$id),array('picture'=>$new_video_preview,'photo_profile'=>$new_user_photo));
								//end update image name
				   	...
				   	//update max and since id in video_status
					 $result_status = $this->search_engine_status_m->get_by(array('via'=>'twitter'));
					 if($result_status)
					 {
						...
						$this->search_engine_status_m->update_by(array('via'=>'twitter'),array('max_id'=>$_max,'since_id'=>$_since_id));
						
					 }
					 else {
						$this->search_engine_status_m->insert(array('max_id'=>$_max,'since_id'=>$_since_id,'via'=>'twitter'));
					 }
					 //end update status video
				 }
				//get old one
				 ...
				 if(isset($data1->{'search_metadata'}->next_results))
				 {
				 	...
				 	if (isset($data2->{'statuses'}))
					{
					 	 ...
					 	 foreach($data2->{'statuses'} as $data_insert1)
						 {
								...
								if(isset($data_insert1->place->full_name))
								{
									$place = $data_insert1->place->full_name;
								}
								/*if( !preg_match('/\/video\/1$/',$video) && $this->debug)
								{
									continue 1;
						 ...
						 if($result_status)
						 {
							...
							$this->search_engine_status_m->update_by(array('via'=>'twitter'),array('max_id'=>$_max_1,'since_id'=>$_since_id_1));
								
							 
						 }
						 else {
							 $this->search_engine_status_m->insert(array('max_id'=>$_max_1,'since_id'=>$_since_id_1,'via'=>'twitter'));								
				...
				//end get old one
						...
						$getfield = '?q='.rawurlencode($this->global_tag).'&include_entities=1&count='.$max_count;
					$requestMethod = 'GET';
					$result = $this->global_twitter->setGetfield($getfield)
		             ->buildOauth($url, $requestMethod)
		             ->performRequest();
			 ...
			 if(isset($data->{'statuses'}))
			 {
				 ...
				 if($result_status)
				 {
					 $this->search_engine_status_m->update_by(array('via'=>'twitter'),array('max_id'=>$_max,'since_id'=>$_max));
				 }
				 else {
				 	$this->search_engine_status_m->insert(array('max_id'=>$_max,'since_id'=>$_max,'via'=>'twitter'));
				 }
				 //end update status video
			 }
