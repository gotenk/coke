 ...
 */
class Admin_black_list_words extends Admin_Controller {
	
	protected $section = 'list_words';
	...
	public function __construct() {
		...
		$this->load->model(array('black_list_word_m'));
		...
		$this->template->set('master_status',array('draft'=>lang('search_engine:draft_label'),'live'=>lang('search_engine:live_label'),'deleted'=>lang('search_engine:delete_label'),'black_listed'=>lang('search_engine:black_listed_label')));
	...
	public function index() {		
		...
		if ($this->input->post('f_keywords')) {
			$this->black_list_word_m->like('name',$this->input->post('f_keywords'));
			//$base_where['name'] = $this->input->post('f_keywords');
		}
			
		$total_rows = $this->black_list_word_m->count_by($base_where);
		...
		$data = $this->black_list_word_m->get_many_by($base_where);
		...
		$this->template
			...
			? $this->template->build('admin/black_list_words/tables/black_list_words')
			: $this->template->build('admin/black_list_words/index');
	...
	function action(){
		...
		redirect(ADMIN_URL.'/search_engine/black_list_words');
	}
	...
	public function create() {		
		...
		if($this->form_validation->run())
		{
			...
			if($id = $this->black_list_word_m->insert((array)$data))
			{
				$this->session->set_flashdata('success',sprintf( lang('search_engine:success_create_master_black_list_word_message'),$data->name ) )  ; 
				...
				$this->template->set('messages',array('error'=>lang('search_engine:failed_create_master_black_list_word_message')));
			}			
	...
	public function edit($id = 0) {
		...
		$data = $this->black_list_word_m->get_by(array('id'=>$id));
		...
		if($this->form_validation->run())
		{
			...
			$success = $this->black_list_word_m->update_by(array('id'=>$new_id),(array)$data);
			if($success)
			{
				$this->session->set_flashdata('success',sprintf( lang('search_engine:success_edit_master_black_list_word_message'),$data->name ) )  ; 
				redirect( (($this->input->post('btnAction') =='save_exit') ? ADMIN_URL.'/search_engine/master_list_name' : ADMIN_URL.'/search_engine/master_list_name/edit/'.$id) );
			}
			else {
				$this->template->set('messages',array('error'=>lang('search_engine:failed_edit_master_black_list_word_message')));
			}		
		}
	...
	function delete(){
		...
		{
			if($id!=0){
				$info_photo = $this->list_word_m->get_by(array('id'=>$id));
				$photo_name = $info_photo->row('name');
				$del = $this->list_word_m->update_by( array('id'=>$id),array('status'=>'deleted'));
					
				if($del){
					$this->session->set_flashdata('success', 'Black list word '.$photo_name.' deleted');
				}else{
					$this->session->set_flashdata('error', 'Black list word '.$photo_name.' can"t delete');
				}
	...
	{		
		...
		return $this->black_list_word_m->check_exists('name', $name, $id);
	}
}
