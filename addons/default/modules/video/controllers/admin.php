...
class Admin extends Admin_Controller {
	...
	protected $validation_rules = array(
		'status' => array(
			'field' => 'status',
			'label' => 'lang:product:status_label',
	...
	public function __construct() {
		...
		$this->load->model(array('bubble_m'));
		$this->lang->load(array('video'));
        $this->load->library('image_lib');
		$this->load->library('facebook');
		$this->load->model('video/bitly_cache_m');
		...
		$this->template
			...
			->append_css('module::dago_gallery.css');
		*/
	...
	public function index() {		
		...
		if ($this->input->post('f_via')) {			
			$base_where['via'] = $this->input->post('f_via');
		}
		...
		$total_rows = $this->bubble_m->get_list_bubble($base_where)->num_rows();
		$pagination = create_pagination(ADMIN_URL.'/video/index', $total_rows); 

		$this->db->limit($pagination['limit'], $pagination['offset']);	
		$data = $this->bubble_m->get_list_bubble($base_where)->result();
		...
		$this->template
			...
			? $this->template->build('admin/tables/bubble')
	...
	function action(){
		...
		if($btnAction=='top'){
			...
			$data_top = $this->bubble_m->cek_top();	
		...
		if(count($id) > 0){
			foreach ($id as $key => $value) {
				if($btnAction=='top'){
					$this->db->update('video', array('id'=>$value));
				}
				...
				if($btnAction=='live'){
					$info_bubble = $this->db->get_where('video', array('id'=>$value));
					...
					if($info_bubble->num_rows() >  0 ){
						...
						$this->db->update('video', $data_update, array('id'=>$value));
							
					}else{
						$this->db->update('video', array('status'=>$btnAction), array('id'=>$value));
					}
					$this->db->update('video', array('status'=>$btnAction), array('id'=>$value));
				}

				if($btnAction=='draft'){
					$info_bubble = $this->db->get_where('video', array('id'=>$value, 'status'=>'live'));
					if($info_bubble->num_rows() >  0 ){
						$new_photo_path = $new_thumb_path = '';
						$drafted = '_'.$value.'_drafted';
	...
	public function create($id_db=0) {
		...
		$data_db = $this->bubble_m->get_cerita_by($id_db)->row();
		$created_on = now();
        $id= 0;
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('id') == "") {
				...
				$cek = $this->db->get_where('video', array('entity_id'=>$data_inputan['entity_id']));
				...
				if($cek->num_rows() ==  0 ) {
					...
					$data_inputan['video'] = $this->input->post('video_url');
					...
					if ($id_db == 0) {
						if ($id = $this->bubble_m->insert_data($data_inputan)) {
							if ($data_inputan['status'] == "draft") {
								...
								$this->db->update('video', array('video_preview'=>'/uploads/default/files/facebook_image/'.$new_filename_video), array('id'=>$id));
								$this->session->set_flashdata('success', sprintf($this->lang->line('video:post_add_success'), $data_inputan['name']));
							} else {
								//tambahan untuk ngebitly
								$info_bubble = $this->db->get_where('video', array('id'=>$id));
								$data_url = site_url(array('galeri','index',1,$info_bubble->row('id')));
						...
						if ($data_inputan['status'] == "draft") {
							$status_curr = $this->db->get_where('video', array('id'=>$id_db, 'status'=>'live'));
							if($status_curr->num_rows() >  0 ) {
								...
								$this->db->update('video', $data_inputan, array('id'=>$id_db));
							} else {
								$this->db->update('video', $data_inputan, array('id'=>$id_db));
							}
						} else {
							$info_bubble = $this->db->get_where('video', array('id'=>$id_db));
							//tambahan untuk ngebitly
							$data_url = site_url(array('galeri','index',1,$info_bubble->row('id')));
					...
					($this->input->post('btnAction') == 'save_exit') ? redirect(ADMIN_URL.'/video') : redirect(ADMIN_URL.'/video/create/'.$id);
				} else {
					$this->session->set_flashdata('error', "Video yang anda isikan sudah ada dalam database.");
					redirect(ADMIN_URL.'/video/create/');
				}
			} else {
	...
	public function edit($id = 0) {
		...
		$id or redirect(ADMIN_URL.'/video');

		$dago_gallery = $this->bubble_m->get_cerita_by($id)->result();
		...
		if ($this->form_validation->run()) { 
			...
			if($id_cerita){
				...
				if($status=='live'){
					$info_bubble = $this->db->get_where('video', array('id'=>$id_cerita));
					//tambahan untuk ngebitly
					$data_url = site_url(array('galeri','index',1,$info_bubble->row('id')));
					...
					if($info_bubble->num_rows() >  0 ){
						...
						$this->db->update('video', $data_update, array('id'=>$id_cerita));							
					}else{
						$this->db->update('video', array('status'=>$status), array('id'=>$id_cerita));
					}
				}

				if($status=='draft'){
					$info_bubble = $this->db->get_where('video', array('id'=>$id_cerita, 'status'=>'live'));
					$this->db->update('video', array('status'=>$status, 'description'=>$description), array('id'=>$id_cerita));
					//tambahan untuk ngebitly
					$data_url = site_url(array('galeri','index',1,$info_bubble->row('id')));
			...
			($this->input->post('btnAction') == 'save_exit') ? redirect(ADMIN_URL.'/video') : redirect(ADMIN_URL.'/video/edit/'.$id);
		}
	...
	function delete(){
		...
		{
			if($id!=0){
				$info_photo = $this->db->get_where('video', array('id'=>$id));
				$photo_name = $info_photo->row('name');
				$del = $this->db->update('video', array('status'=>'deleted'), array('id'=>$id));
				if($info_photo->num_rows() >  0 ){
					...
					$this->db->update('video', $data_update, array('id'=>$id));
				}
				...
				redirect(site_url(ADMIN_URL.'/video'));
			}
		}
		else {
			redirect(site_url(ADMIN_URL.'/video'));
	...
	function export_fb_crawling($status=0, $sorted=0, $keyword=0){
		$this->load->library('excel');
			...
			$data = $this->db->get_where('video', array('via_fb'=>1, 'was_import'=>0, 'author_id'=>0, 'status !='=>'deleted'));
			if($data->num_rows <=0){
            ...
            $this->excel->getActiveSheet()->setCellValue('C1', 'video');
            ...
            foreach($data->result() as $dt){
            	$this->db->update('video', array('was_import'=>1, 'date_import'=>date('Y-m-d')), array('id'=>$dt->id));
	...
	public function facebook_user() {
		...
		$total_rows = $this->bubble_m->get_list_fb_user($base_where)->num_rows();
		$pagination = create_pagination(ADMIN_URL.'/video/index', $total_rows); 

		$this->db->limit($pagination['limit'], $pagination['offset']);	
		$data = $this->bubble_m->get_list_fb_user($base_where)->result();
	...
	public function facebook_video($fb_id = 0) {		
		...
		$total_rows = $this->bubble_m->get_list_fb_video($base_where)->num_rows();
		$pagination = create_pagination(ADMIN_URL.'/video/index', $total_rows, 25, 5);
		//var_dump($this->db->last_query());

		$this->db->limit($pagination['limit'], $pagination['offset']);	
		$data = $this->bubble_m->get_list_fb_video($base_where)->result();
		//var_dump($this->db->last_query());
		$this->input->is_ajax_request() and $this->template->set_layout(false);
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('id') == "") {
				...
				if($cek->num_rows() ==  0 ) {
					...
					($this->input->post('btnAction') == 'save_exit') ? redirect(ADMIN_URL.'/video/facebook_video/'.$user_id) : redirect(ADMIN_URL.'/video/fb_create/'.$user_id.'/'.$id);
				} else {
					$this->session->set_flashdata('error', "Video yang anda isikan sudah ada dalam database.");
					redirect(ADMIN_URL.'/video/fb_create/'.$user_id);
				}
			} else {
				$id_db = $this->input->post('id');
					$token = $this->db->get_where('settings', array('slug'=>'fb-token'));
	...
	function fb_delete($id=0){
		if($id!=0){
			...
			redirect(site_url(ADMIN_URL.'/video/facebook_video/'.$info_photo->row('userid')));
		}
	...
	function fb_action(){
		...
		redirect(ADMIN_URL.'/video/facebook_video/'.$this->input->post('fb_id'));
	}
