        ...
        <div id="input-inner">
            <div id="tab-1" class="tab-container current">
                <span class="panel">
                    ...
                    <input type="text" placeholder="ketik kode unik di sini" name="alfamart_code" />
                </span> <!-- .panel -->
                <span class="panel">
                    <label>kode transaksi</label>
                    <input type="text" placeholder="ketik kode transaksi di sini" name="transaction_code" />
                </span> <!-- .panel -->
            </div> <!-- #vendor-a -->
            <div id="tab-2" class="tab-container">
                <span class="panel">
                    <label>kode unik</label>
                    <input type="text" placeholder="ketik kode unik di sini" name="indomaret_code" />
                </span> <!-- .panel -->
            </div> <!-- #vendor-b -->
            <div class="psst">
                <?php if ($this->session->flashdata('message')) { ?>
                    <span class="error-m">
                        <?php foreach ($this->session->flashdata('message') as $error) { ?>
                            <?= $error; ?>
                        <?php } ?>
                    </span><!-- .error-m -->
                <?php } ?>
