...
(function(jQuery)
	{ 
		jQuery.fn.dateSelectBoxes = function(monthElem, dayElem, yearElem, keepLabels,use_multiple )
			{
				if(use_multiple == true)
				{
					if (keepLabels) {
						var dayLabel = dayElem.eq(0).find('option:eq(0)').val();
					}
					...
					function isLeapYearMM($pos) {
						var selected = yearElem.eq($pos).selectedValues();
						return ( selected === "" || ( ( selected % 4 === 0 ) && ( selected % 100 !== 0 ) ) || ( selected % 400 === 0) );
					}
					function updateDaysMM($from_year,$pos,$update_current) {
						var selected = dayElem.eq($pos).selectedValues(), days = [], i;
						dayElem.eq($pos).removeOption(/./);
						var month = parseInt(monthElem.eq($pos).val(),10);
						if (!month) {
							//Default to 31 days if no month selected.
							month = 1;
							//selected = ['hari','hari'];
						}
						var year = parseInt(yearElem.eq($pos).val(),10);
						if (!year && $from_year == true) {
							//monthElem.eq($pos).selectOptions('bulan');
							//selected = ['hari','hari'];
						}
						switch (month) {
							case 1:
						...
						if($update_current == false)
						{
							...
							dayElem.eq($pos).addOption(days, false);
							dayElem.eq($pos).selectOptions(selected.shift());
						}
						else if($update_current == true)
						{
							
							dayElem.eq($pos).addOption(days, false);
							dayElem.eq($pos).selectOptions(selected.shift());
							//update siblings value
							$next_pos = (($pos == 0) ? 1 : 0); 
							//console.log($next_pos);
							//set value
							/*console.log('y',yearElem.eq($pos).val());
							console.log('m',monthElem.eq($pos).val());
							console.log('d',dayElem.eq($pos).val());*/
							yearElem.eq($next_pos).selectOptions(yearElem.eq($pos).val());
							monthElem.eq($next_pos).selectOptions(monthElem.eq($pos).val());
							dayElem.eq($next_pos).selectOptions(dayElem.eq($pos).val());
							//trigger value
							updateDaysMM(null,$next_pos,false);
							
						}
						
						
					}
					dayElem.change(function(){
						$eq = dayElem.index($(this));
						updateDaysMM(true,$eq,true);
					});
					yearElem.change( function() {
						$eq = yearElem.index($(this));
						updateDaysMM(true,$eq,true);
					});
					monthElem.change( function() {
						$eq = monthElem.index($(this));
						updateDaysMM(null,$eq,true);
					});
				}
				else
				{
					...
					function updateDays($from_year) {
						...
						var year = parseInt(yearElem.val(),10);
						if (!year && $from_year == true) {
							//Default to 31 days if no month selected.
							monthElem.selectOptions('bulan');
							selected = ['hari'];
						}
						switch (month) {
							case 1:
						...
						dayElem.selectOptions(selected.shift());
