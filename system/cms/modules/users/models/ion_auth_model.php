...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function forgotten_password_id($id = '')
	{
	    if (empty($id))
	    {
			return false;
		...
		$key = $this->hash_password(microtime().$id);

		$this->forgotten_password_code = $key;

		$this->db->where($this->ion_auth->_extra_where);

		$this->db->update($this->tables['users'], array('forgotten_password_code' => $key), array('id' => $id));
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $valid_email=null, $group_id = null, $additional_data = array(), $group_name = false,$my_extra=array())
	{
		...
		$extra = array_merge($extra,$my_extra);
	...
	public function login($identity, $password, $remember=false)
	{
		...
		$this->db->select('username, email, id, password, group_id,active')
			->where(sprintf('(username = "%1$s" OR email = "%1$s")', $this->db->escape_str($identity)));
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($user->password === $password)
			{
				...
				{ 
					...
					//var_dump(intval($profile_dat->address));
					//var_dump((intval($profile_dat->address) >= $max_attempt_login) && $user->active == 1);die();
					//if((intval($profile_dat->address) >= $max_attempt_login) && $user->active == 1 )
					if((intval($profile_dat->password_temp) >= $max_attempt_login) && $user->active == 1 )
					{
						$this->db->where(array('id'=>$user->id));
						$this->db->update($this->tables['users'],array('active'=>0));
	...
	public function get_users($group = false, $limit = null, $offset = null)
	{
		...
		$this->db->select($this->tables['meta'].'.color_class as color_class');
		$this->db->select($this->tables['meta'].'.photo_profile as photo_profile');
		$this->db->select($this->tables['meta'].'.complete_challenge as complete_challenge');
	...
	public function update_user($id, $data, $profile_data)
	{
		...
		$profile_parsed_data['dob'] 	= (array_key_exists('dob', $profile_data)) ? $profile_data['dob'] : $profile->dob;
		$profile_parsed_data['dob_date_format'] 	= (array_key_exists('dob_date_format', $profile_data)) ? $profile_data['dob_date_format'] : $profile->dob_date_format;
		$profile_parsed_data['insta_id'] 	= (array_key_exists('insta_id', $profile_data)) ? $profile_data['insta_id'] : $profile->insta_id;
		$profile_parsed_data['vine_id'] 	= (array_key_exists('vine_id', $profile_data)) ? $profile_data['vine_id'] : $profile->vine_id;
		$profile_parsed_data['fb_id'] 	= (array_key_exists('fb_id', $profile_data)) ? $profile_data['fb_id'] : $profile->fb_id;
		$profile_parsed_data['tw_id'] 	= (array_key_exists('tw_id', $profile_data)) ? $profile_data['tw_id'] : $profile->tw_id;
		$profile_parsed_data['tw_screenname'] 	= (array_key_exists('tw_screenname', $profile_data)) ? $profile_data['tw_screenname'] : $profile->tw_screenname;
